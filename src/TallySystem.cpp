...
void setupTallySystem(char* settings_password) {
    ...
    server.on("/api/readsettings",[settings_password]() {
        if(!server.authenticate("tallylight", settings_password)) return server.requestAuthentication();
        server.send(200, "application/json", "{ \"host\" : \""+String(cfg1.tallyHost)+"\", \"key\" : \""+String(cfg1.tallyKey)+"\", \"ip\" : \""+WiFi.localIP().toString()+"\" }");
        return;
    });
    
    server.on("/api/writesettings",[settings_password]() {
        if(!server.authenticate("tallylight", settings_password)) return server.requestAuthentication();
        if(server.arg("host").length() > 0) {
            strcpy(cfg1.tallyHost, server.arg("host").c_str());
        }
        if(server.arg("key").length() > 0) {
            strcpy(cfg1.tallyKey, server.arg("key").c_str());
        }
        saveTallyConfig();
        server.send(200, "application/json", "{ \"host\" : \""+String(cfg1.tallyHost)+"\", \"key\" : \""+String(cfg1.tallyKey)+"\", \"ip\" : \""+WiFi.localIP().toString()+"\" }");
        return;
...
void handleClient_TallyLight() {
    ...
    if(runTally && !isRequestOpen) {
        String url = String(cfg1.tallyHost)+"/tallyupdate/?key="+String(cfg1.tallyKey)+"&state=%23"+currentValue;
